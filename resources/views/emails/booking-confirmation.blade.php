...
<head>
    ...
    <title>Demande de réservation envoyée</title>
    <style>
        ...
        .badge { display: inline-block; background: #ecfdf5; color: #059669; padding: 4px 12px; border-radius: 9999px; font-size: 12px; font-weight: 600; letter-spacing: 0.4px; }
...
<body>
    <div class="card">
        <div class="badge">Demande envoyée</div>
        <h1>Votre demande a bien été reçue</h1>
        <p class="sub">
            Bonjour {{ $booking->name }}, votre demande de reservation aupres de
            <strong>{{ $booking->autoSchool->name }}</strong> a ete transmise. L'auto-ecole vous recontactera prochainement.
        </p>

        <div class="info-row">
            <span class="info-label">Auto-ecole</span>
            <span class="info-value">{{ $booking->autoSchool->name }}</span>
        </div>
        ...
        <div class="info-row">
            <span class="info-label">Date souhaitee</span>
            <span class="info-value">{{ $booking->preferred_date->format('d/m/Y') }}</span>
        </div>
        @endif

        <hr class="divider">
        <a href="{{ route('school.detail', $booking->autoSchool->slug) }}" class="btn">Voir la fiche de l'ecole</a>
